...
#include <algorithm>// for http://en.cppreference.com/w/cpp/algorithm/max function
// #include <iostream>
#include "main.h"
#include "model.h"
#include "script.h"
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE hinst,LPSTR pStr,int nCmd)
{
    ...
    //SYSTEM_INFO siSysInfo;     * replaced with std::thread::hardware_concurrency() function
    //GetSystemInfo(&siSysInfo); *
    num_cores = max(1U, std::thread::hardware_concurrency());
    ...
    switch(message)
    {
        ...
        case WM_LBUTTONUP:
            ...
            if(Popup->floating_itembar==SLOT_DOWNLOAD)
            {
                #ifdef USE_TORRENT
                if(Updater->isSeedingDrivers())Updater->StopSeedingDrivers();
//SDIO          else if(Updater->isSeedingDownloads())Updater->StopSeedingDownloads();
