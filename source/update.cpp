...
public:
    ...
    //bool isSeedingDownloads(); commented by Delahoy
...
void UpdateDialog_t::setCheckboxes()
{
    ...
    if(h.file_priority(i)==2)
    {
        if(StrStrIA(h.torrent_file()->file_at(i).path.c_str(),"indexes\\"))
            indexesChecked=1;
        else
            baseChecked=1;
    }
...
void UpdateDialog_t::setPriorities()
{
    ...
    if(StrStrIA(h.torrent_file()->file_at(i).path.c_str(),"drivers\\"))
        h.file_priority(i,0);
    ...
    for(int i=0;i<ListView.GetItemCount();i++)
    {
        ...
        if(ItemData->DefaultSort>= 0)h.file_priority(static_cast<int>(ItemData->DefaultSort),val);
    }

    // Set priorities for any torrent file that's not a driver
    for(file_index_t i(0);i<Updater->numfiles;i++)
    if(!StrStrIA(h.torrent_file()->file_at(i).path.c_str(),"drivers\\"))
        h.file_priority(i,StrStrIA(h.torrent_file()->file_at(i).path.c_str(),"indexes\\")?indexes_pri:base_pri);
}
...
BOOL CALLBACK UpdateDialog_t::UpdateProcedure(HWND hwnd,UINT Message,WPARAM wParam,LPARAM lParam)
{
    ...
    switch(Message)
    {
        ...
        case WM_TIMER:
            if(ses.is_paused())UpdateDialog.populate(1);
            //Log.print_con(".");
            break;
...
int UpdateDialog_t::populate(int update,bool clearlist)
{
    ...
    Updater->numfiles = file_index_t(ti->num_files());
    ...
    for(file_index_t i(0); i<Updater->numfiles; ++i)
    {
        ...
        else if(!StrStrIA(filenamefull,"drivers\\"))
        {
            ...
            if(StrStrIA(filenamefull,"sdi_R"))
                TorrentRevision=atol(StrStrIA(filenamefull,"sdi_R")+5);
        }
    }
...
//if(TorrentRevision>0)TorrentRevision=700;  // test
    ...
    for(int i=0;i<Updater->numfiles;i++)
    {
        ...
        lt::file_storage st = ti->files();
        filenamefull=strchr(st.file_path(i).c_str(), '\\') + 1;
        ...
        if(StrStrIA(filenamefull,".7z"))
        {
            ...
            lvI.pszText=buf;
            int sz=(int)(st.file_size(i)/1024/1024);
            if(!sz)sz=1;
...
void UpdateDialog_t::setFilePriority(const wchar_t *name,int pri)
{
    ...
    for(int i=0;i<Updater->numfiles;i++)
    if(StrStrIA(h.torrent_file()->file_at(i).path.c_str(),buf))
    {
        h.file_priority(i,pri);
...
void UpdaterImp::updateTorrentStatus()
{
    ...
    temp = ses.get_torrent_status(&yes1, {});
...
void UpdaterImp::moveNewFiles()
{
    ...
    {
        lt::file_storage st = ti->files();
        std::string filenamefull=st.file_path(i);
        if(activetorrent==1)
            filenamefull=strchr(st.file_path(i).c_str(),'\\')+1;
        ...
        wsprintf(filenamefull_src,L"%s\\%S", active_torrent_save_path->c_str(),st.file_path(i).c_str());
...
void UpdaterImp::ShowProgress(wchar_t *buf)
{
    ...
    if(ses.is_valid())
    {
        ...
        std::vector<lt::torrent_status> temp = ses.get_torrent_status(&yes1, {});
        ...
        lt::torrent_status& st = temp[0];
            ...
            else if (st.state == lt::torrent_status::queued_for_checking)
            ...
            else if (st.state == lt::torrent_status::downloading_metadata)
                wsprintf(buf, STR(STR_TR_ST2));
            else if (st.state == lt::torrent_status::downloading)
            ...
            else if (st.state == lt::torrent_status::finished)
                wsprintf(buf, STR(STR_TR_ST4));
            else if (st.state == lt::torrent_status::seeding)
                wsprintf(buf, STR(STR_DWN_SEEDING), num3, num4);
            else if (st.state == lt::torrent_status::allocating)
...
bool UpdaterImp::add_torrent(lt::session& ses, std::string torrent)
{
    using lt::add_torrent_params;
    using lt::storage_mode_t;

    Log.print_con("Torrent: %s\n", active_torrent_url->c_str());
    ...
    p.flags = add_torrent_params::flag_paused | add_torrent_params::flag_seed_mode | add_torrent_params::flag_auto_managed;
    p.url = torrent;
...
int UpdaterImp::downloadTorrent()
{
    ...
    pack.set_bool(settings_pack::enable_lsd, true); //s->start_lsd(); - deprecated 
    ...
    pack.set_str(settings_pack::user_agent, "Snappy Driver Installer " "SDI_VERSIONINFO" LIBTORRENT_VERSION);
 ...
 //   settings.use_dht_as_fallback=false; Is false by default
 //   settings.broadcast_lsd=true;  Broadcast messages might be expensive and disruptive on networks, deprecated in 1.2.4. Just use multicast
    ses.add_dht_router(std::make_pair(std::string("router.bittorrent.com"),6881));
    ses.add_dht_router(std::make_pair(std::string("router.utorrent.com"),6881));
    ses.add_dht_router(std::make_pair(std::string("router.bitcomet.com"),6881));
    ses.start_dht();
 

//   settings.always_send_user_agent=true; Include a user-agent to just the first request in a connection
 //   settings.anonymous_mode=false;     Default is false
 //   settings.set_bool(settings_pack::allow_multiple_connections_per_ip, true);            //*That way two people from behind the same NAT
...
//    s->set_settings(settings); That function is deprecated lt::settings_pack pack;
    lt::session ses(pack);
   
    // Connecting
    ses.listen_on(std::make_pair(torrentport,torrentport),ec);
  ...
  if(ec)Log.print_err("ERROR: failed to add torrent: %s\n",ec.message().c_str());
    ...
    Log.print_con("Updated DriverPacks available: %d\n",i&0xFF);

    // clear the torrent priorities
    for(int j=0;j<numfiles;j++)h.file_priority(j,0);
...
int UpdaterImp::scriptDownloadApp()
{
    ...
    {
        if(!(StrStrIA(h.torrent_file()->file_at(i).path.c_str(),"indexes\\"))&&
           !(StrStrIA(h.torrent_file()->file_at(i).path.c_str(),"drivers\\")))
            h.file_priority(i,2);
        else
            h.file_priority(i,0);
    }
...
int UpdaterImp::scriptDownloadIndexes()
{
    ...
    {
        // get the file entry
        lt::file_entry fe=h.torrent_file()->file_at(i);
...
int UpdaterImp::scriptDownloadDrivers(std::wstring mode)
{
    ...
    if(!updatecount)
    {
        Log.print_con("DriverPacks are up to date, nothing to do\n");
        return 0;
    }

    Log.print_con("Getting %d DriverPacks\n",updatecount);
    return scriptDoDownload();
}
...
void UpdaterImp::DownloadNetwork()
{
    for(int i=0;i<Updater->numfiles;i++)
    {
        ...
        else
        {
            ...
            std::string filename=strrchr(h.torrent_file()->file_at(i).path.c_str(),'\\')+1;
        ...
        while(downloadmangar_exitflag==DOWNLOAD_STATUS_DOWLOADING_DATA&&ses.is_valid())
        {
            ...
            if((TorrentStatus.status_strid==STR_TR_ST0+libtorrent::torrent_status::finished)&&!SeedMode)
            {
                ...
                while(1)
                {
                    ...
                    auto const* a=ses.wait_for_alert(lt::seconds(60*2));
                    if(!a)
                    {
                        Log.print_con("time out\n");
                        break;
                    }
                    //lt::alert const* holder2=s->pop_alerts;
...
int Updater_t::uplimit=0;
int Updater_t::connections=0;
std::string Updater_t::torrent_url[BUFSIZ];
#endif
