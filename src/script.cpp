...
#include <windows.h>
#include <shellapi.h>       // for CommandLineToArgvW, instead cumbersome setupapi.h
#include <iostream>
#include "settings.h"
#include "system.h"
                    ...
                    else if(_wcsicmp(args[1].c_str(),L"DriverPacks")==0)
                    {
                        if(args.size()>2)
                        {
                               ...
                               _wcsicmp(args[2].c_str(),L"selected")==0)
                            {
                                ...
                                if(!LastExitCode)Log.print_con("DriverPacks downloaded successfully\n");
                                else Log.print_con("DriverPacks download failed\n");
                            }
                            else
                            {
                                LastExitCode=1;
...
void Script::RunLatest(std::wstring args)
{
    ...
    std::wstring cmd;
    #ifdef _WIN64
        cmd=L"SDI_x64_R"+std::to_wstring(System.FindLatestExeVersion(64))+L".exe";
    #else
        cmd=L"SDI_R"+std::to_wstring(System.FindLatestExeVersion(32))+L".exe";
    #endif // _WIN64
