...
void VaultLang::EnumFiles(Combobox *lst,const wchar_t *path,int locale)
{
		...
		if(!(FindFileData.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY))
		{
				...
				if((((std::get<int>(language[STR_LANG_CODE].value)&0xFF00)==0)&&std::get<int>(language[STR_LANG_CODE].value)==(locale&0xFF))||
					 (std::get<int>(language[STR_LANG_CODE].value)==locale))
				{
						lang_auto_str.sprintf(L"Auto (%s)",STR(STR_LANG_NAME));
						lang_auto=i;
				}
				lst->AddItem(STR(System.IsLangInstalled(std::get<int>(language[STR_LANG_GROUP].value)) ? STR_LANG_NAME : STR_LANG_ID));
