...
int State::getbaseboard(WStringShort &manuf1,WStringShort &model1,WStringShort &product1,WStringShort &cs_manuf1,WStringShort &cs_model1,int *type)
{
    *type=0;
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to initialize COM library\n");
        return 0;
    }
    ...
    if(!initsec)
    {
        ...
        /*if(FAILED(hres))
        {
            uprintf("FAILED to initialize security. Error code = 0x%lX\n",hres);
            //CoUninitialize();
            return 0;
        }*/
    }
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to create IWbemLocator object. Error code = 0x%lX\n",hres);
        //CoUninitialize();
        return 0;
    }
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to connect to root\\cimv2. Error code = 0x%lX\n",hres);
        pLoc->Release();
        //CoUninitialize();
        return 0;
    }
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to set proxy blanket. Error code = 0x%lX\n",hres);
        pSvc->Release();
        pLoc->Release();
        //CoUninitialize();
        return 0;
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to query for Win32_BaseBoard. Error code = 0x%lX\n",hres);
        pSvc->Release();
        pLoc->Release();
        //CoUninitialize();
        return 0;
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to query for Win32_ComputerSystem. Error code = 0x%lX\n",hres);
        pSvc->Release();
        pLoc->Release();
        //CoUninitialize();
        return 0;
    ...
    if(FAILED(hres))
    {
        uprintf("FAILED to query for Win32_SystemEnclosure. Error code = 0x%lX\n",hres);
        pSvc->Release();
        pLoc->Release();
        //CoUninitialize();
        return 0;
