...
void UpdaterImp::ShowProgress(wchar_t *buf)
{
		...
		if(ses.is_valid())
		{
				...
				if(closingsession)
						...
						wsprintf(buf,STR(STR_DWN_SEEDING),num3,num4);
				//else if(st.state==torrent_status::)							 //deprecated
...
int UpdaterImp::downloadTorrent()
{
	// checking for updates
		...
		if(ec)Log.print_err("ERROR: failed to open listen socket: %ses\n",ec.message().c_str());
...
void UpdaterImp::StartSeedingDrivers()
{
		...
		// once i've done this it'ses no good for regular downloading
